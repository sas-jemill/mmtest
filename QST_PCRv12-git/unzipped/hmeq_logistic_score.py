...
def scoreHMEQLogisticModel(JOB, REASON, CLAGE, CLNO, DEBTINC, DELINQ, DEROG, NINQ, YOJ):
    ...
    output_table = pandas.DataFrame({
        "EM_EVENTPROBABILITY": _predProb.iloc[:,1], 
        "EM_CLASSIFICATION": numpy.where(_predProb.iloc[:,1] >= _threshPredProb, 1, 0)
    })
    if isinstance(JOB, pandas.Series):
        return output_table
    else:
        return tuple(output_table.values.squeeze())